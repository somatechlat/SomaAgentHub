...
class FakeAsyncClient:
    ...
    async def post(self, url, json=None, headers=None):  # noqa: A003
        if url.endswith('/v1/build/precheck'):
            ...
            return Response(200, content=json.dumps(data), headers={'Content-Type': 'application/json'})
...
def test_cost_precheck(monkeypatch):
    ...
    resp = client.post('/v1/build/cost-precheck', json={
        'project_id': 'proj1',
        'hours_planned': 1.0,
        'quantity': 1,
        'budget_cap': 10.0
    }, headers={'X-Tenant-ID': 'demo', 'X-Client-Type': 'web', 'X-Deployment-Mode': 'developer-light'})
...
def test_build_run_requires_snapshot_auto(monkeypatch):
    ...
    monkeypatch.setenv('SOMAGENT_GATEWAY_ORCHESTRATOR_URL', 'http://orchestrator-mock')
    monkeypatch.setattr(httpx, 'AsyncClient', FakeAsyncClient)
    resp = client.post('/v1/build/run', json={
        'project_id': 'proj1',
        'budget_cap': 100.0,
        'estimated_cost': 10.0
    }, headers={'X-Tenant-ID': 'demo', 'X-Client-Type': 'web', 'X-Deployment-Mode': 'developer-light'})
