...
class FakeAsyncClient:
    ...
    async def post(self, url, params=None, json=None, headers=None, timeout=None):  # noqa: A003
        ...
        if url.endswith('/v1/mao/start'):
            data = {'workflow_id': 'wf-1', 'orchestration_id': 'orc-1', 'task_queue': 'q'}
            return httpx.Response(200, content=json.dumps(data), headers={'Content-Type': 'application/json'})
        return httpx.Response(404, content=json.dumps({'detail': 'not found'}), headers={'Content-Type': 'application/json'})
