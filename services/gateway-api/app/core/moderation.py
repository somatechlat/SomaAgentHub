...
class ModerationGuard:
    ...
    def _strike_key(self, tenant_id: str, user_id: str | None) -> str:
        """Construct the Redis key used for storing strike counters.

        The key combines the configured prefix, tenant identifier and an
        optional user identifier (or ``anonymous`` when ``user_id`` is ``None``).
        """
...
def get_moderation_guard() -> ModerationGuard:
    ...
    shared Redis client provided by :func:`get_redis_client`. Subsequent calls
    return the already‑instantiated guard.
    """
