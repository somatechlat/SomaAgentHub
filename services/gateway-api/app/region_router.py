...
class Region(str, Enum):
    ...
    US = US_WEST_2  # Fallback aliases using existing enum values
    EU = EU_WEST_1
...
class RegionRouter:
    ...
    def get_region_for_user(
        ...
        if data_zone == DataResidencyZone.EU:
            ...
            # Fallback for APAC to closest available region if AP_SOUTHEAST_1 not in config
            if Region.AP_SOUTHEAST_1 in REGION_CONFIG:
                return Region.AP_SOUTHEAST_1
            return Region.US_WEST_2  # Default fallback
    ...
    def validate_data_residency(self, user_id: str, target_region: Region) -> bool:
        ...
        # Guard against key error if region not in config
