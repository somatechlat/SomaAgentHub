...
class GraphWorkflowEngine:
    ...
    def __init__(self):
        self.client = None
        self.worker = None
        self.active_workflows = {} # In-memory cache of active workflow metadata (could be Redis/DB in prod)
...
class GraphWorkflowDef:
    ...
    async def execute_graph(self, workflow_id: str, graph_def: Dict[str, Any], initial_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while queue:
            ...
            for i, node_id in enumerate(current_layer):
                ...
                if isinstance(result, dict) and result.get("status") == "failed":
                    return {"status": "failed", "reason": f"Node {node_id} failed", "results": results}
