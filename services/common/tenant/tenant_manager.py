...
class MultiTenantManager:
    ...
    async def create_tenant(
        ...
        resource_quota: TenantResourceQuota | None = None,
    ) -> TenantContext:
    ...
    async def get_tenant_context(self, tenant_id: str) -> TenantContext | None:
        try:
    ...
    async def get_tenant_by_slug(self, slug: str) -> TenantContext | None:
    ...
    async def check_resource_availability(
        self, tenant_context: TenantContext, resource_request: dict[str, Any]
    ) -> bool:
        ...
        if current_usage.get(
            "current_builds", 0
        ) + requested_builds > tenant_context.resource_limits.get(
        ...
        if current_usage.get(
            "current_agents", 0
        ) + requested_agents > tenant_context.resource_limits.get(
            "max_agents_per_build", 10
        ):
            return False
    ...
    async def allocate_resources(
        self, tenant_context: TenantContext, resource_allocation: dict[str, Any]
    ) -> bool:
        tenant_id = tenant_context.tenant_id

        if not await self.check_resource_availability(
            tenant_context, resource_allocation
        ):
            return False
        ...
        logger.info(
            f"Allocated resources for tenant {tenant_id}: {resource_allocation}"
        )
    ...
    async def release_resources(
        ...
        # Update resource usage
        current_usage["current_builds"] = max(
            0,
            current_usage.get("current_builds", 0)
            - resource_allocation.get("builds", 0),
        )
        current_usage["current_agents"] = max(
            0,
            current_usage.get("current_agents", 0)
            - resource_allocation.get("agents", 0),
        )
        current_usage["storage_used_gb"] = max(
            0,
            current_usage.get("storage_used_gb", 0)
            - resource_allocation.get("storage_gb", 0),
        )
        current_usage["cpu_used_cores"] = max(
            0,
            current_usage.get("cpu_used_cores", 0)
            - resource_allocation.get("cpu_cores", 0),
        )
        current_usage["memory_used_gb"] = max(
            0,
            current_usage.get("memory_used_gb", 0)
            - resource_allocation.get("memory_gb", 0),
        )
        current_usage["monthly_cost"] = max(
            0,
            current_usage.get("monthly_cost", 0.0)
            - resource_allocation.get("cost", 0.0),
        )
