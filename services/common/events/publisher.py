"""Centralised event‑bus publishing utilities.

All services should import :class:`EventPublisher` from this module instead of
instantiating ``KafkaClient`` directly.  The class obtains a singleton
``KafkaClient`` via :func:`services.common.kafka_client.get_kafka_client`
and provides a thin, type‑checked API for sending audit, workflow and generic
events.

The implementation deliberately avoids any side‑effects on import – the Kafka
producer is started lazily the first time an event is published.  This mirrors
the behaviour of the original ``KafkaClient`` while giving a consistent entry
...
class EventPublisher:
    """Utility wrapper around the shared :class:`KafkaClient`.
    ...
    The class methods are ``@staticmethod`` so they can be called without
    instantiating the class – this matches the existing pattern used throughout
    the repository (e.g., ``tool_registry``).
    """

    @staticmethod
    async def _client() -> Any:
        """Return a ready ``KafkaClient`` instance.

        The underlying client is started on first use.  If the environment does
        not provide the required Kafka configuration, a ``RuntimeError`` is
        raised – this mirrors the behaviour of ``get_kafka_client``.
        """

        client = get_kafka_client()
        # ``KafkaClient`` implements ``start`` as an async method; ensure it is
    ...
    async def publish_audit(
        *,
        ...
        Returns the generated ``event_id`` for correlation.
        """

        client = await EventPublisher._client()
        return await client.send_audit_event(
    ...
    async def publish_workflow(
        *,
        ...
        Mirrors :meth:`KafkaClient.send_workflow_event`.
        """

        client = await EventPublisher._client()
        return await client.send_workflow_event(
    ...
    async def publish_generic(
        *,
        ...
        """Publish an arbitrary JSON event to *topic*.

        ``event`` should already contain any desired ``event_id``; if omitted a
        UUID will be generated by the underlying ``KafkaClient``.
        """

        client = await EventPublisher._client()
        return await client.send_event(topic=topic, event=event, key=key)
"""
