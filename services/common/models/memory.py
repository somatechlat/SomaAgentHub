...
class MemoryBindingSpec(Base):
    ...
    somabrain_memory_bank_refs = Column(
        JSONB, nullable=False, default=list
    )  # List of ExternalRef IDs
    somabrain_example_store_ref_id = Column(
    ...
    )  # global, per-role, per-node configs

    # Policies
    write_policy = Column(
        JSONB, nullable=False, default=dict
    )  # What gets persisted (RAW, SUMMARIZED, ANONYMIZED)
    read_policy = Column(
        JSONB, nullable=False, default=dict
    )  # Which types of memories allowed
...
class MemoryBindingSpecCreate(BaseModel):
    ...
    somabrain_example_store_ref_id: PyUUID | None = None
...
class MemoryBindingSpecResponse(BaseModel):
    ...
    somabrain_example_store_ref_id: PyUUID | None
    scopes: dict[str, Any]
