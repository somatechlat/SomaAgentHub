...
class MCPServerDefinition(Base):
    ...
    available_tools = Column(JSONB, nullable=False, default=list)  # List of tool IDs/names
...
class ToolInvocationRecord(Base):
    ...
    # Status
    status = Column(SQLEnum(ToolInvocationStatus), nullable=False, default=ToolInvocationStatus.PENDING, index=True)
    ...
    # Policy & Guardrails
    policy_decision = Column(SQLEnum(PolicyDecision), nullable=False, default=PolicyDecision.ALLOWED)
    guardrail_flags = Column(JSONB, nullable=False, default=list)  # Triggered guardrails
...
class ToolInvocationRecordCreate(BaseModel):
    ...
    request_payload_inline: Optional[Dict[str, Any]] = None
...
class ToolInvocationRecordResponse(BaseModel):
    ...
    capsule_instance_id: PyUUID
    request_payload_ref: Optional[str]
    request_payload_inline: Optional[Dict[str, Any]]
    response_payload_ref: Optional[str]
    response_payload_inline: Optional[Dict[str, Any]]
