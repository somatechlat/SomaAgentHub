...
class CapsuleDefinition(Base):
    ...
    status = Column(SQLEnum(CapsuleStatus), nullable=False, default=CapsuleStatus.DRAFT, index=True)
    description = Column(Text, nullable=True)
    
    # Persona Section
    default_persona_ref_id = Column(UUID(as_uuid=True), ForeignKey("external_refs.id"), nullable=True)
    role_overrides = Column(JSONB, nullable=False, default=dict)  # role_id -> persona_ref_id
    
    # Tool Policy Section
    allowed_tools = Column(JSONB, nullable=False, default=list)  # List of tool identifiers
    prohibited_tools = Column(JSONB, nullable=False, default=list)
    allowed_mcp_servers = Column(JSONB, nullable=False, default=list)  # List of MCP server identifiers
    tool_risk_profile = Column(JSONB, nullable=False, default=dict)  # tool_id -> risk_level
    
    # Runtime Section
    max_wall_clock_seconds = Column(Integer, nullable=False, default=300)
    max_concurrent_nodes = Column(Integer, nullable=True)
    allowed_runtimes = Column(JSONB, nullable=False, default=list)  # List of AllowedRuntime
    ...
    egress_mode = Column(SQLEnum(EgressMode), nullable=False, default=EgressMode.ALLOW_LIST)
    
    # Policy Section
    opa_policy_packages = Column(JSONB, nullable=False, default=list)  # List of OPA package IDs
    guardrail_profiles = Column(JSONB, nullable=False, default=list)  # List of guardrail configs
    
    # HITL Section
    default_hitl_mode = Column(SQLEnum(HITLMode), nullable=False, default=HITLMode.ON_HIGH_RISK)
    risk_thresholds = Column(JSONB, nullable=False, default=dict)  # risk_level -> HITL requirement
    ...
    rl_export_scope = Column(SQLEnum(RLExportScope), nullable=False, default=RLExportScope.ANONYMIZED_ONLY)
    rl_excluded_fields = Column(JSONB, nullable=False, default=list)  # Field paths to exclude
    example_store_policy = Column(JSONB, nullable=False, default=dict)  # Constraints for examples
    
    # Compliance Section
    data_classification = Column(SQLEnum(DataClassification), nullable=False, default=DataClassification.INTERNAL)
...
class CapsuleInstance(Base):
    ...
    # Link to definition
    capsule_definition_id = Column(UUID(as_uuid=True), ForeignKey("capsule_definitions.id"), nullable=False, index=True)
    capsule_definition_version = Column(Integer, nullable=False)
    
    # Scope binding
    scope = Column(SQLEnum(CapsuleScope), nullable=False, index=True)
    scope_reference = Column(Text, nullable=False)  # Task ID, Workflow ID, Node ID, or Role name
    ...
    derived_from_id = Column(UUID(as_uuid=True), ForeignKey("capsule_instances.id"), nullable=True)
...
class CapsuleDefinitionCreate(BaseModel):
    ...
    max_pending_hitl: Optional[int] = None
...
class CapsuleDefinitionResponse(BaseModel):
    ...
    max_concurrent_nodes: Optional[int]
...
class CapsuleInstanceCreate(BaseModel):
    ...
    derived_from_id: Optional[PyUUID] = None
...
class CapsuleInstanceResponse(BaseModel):
    ...
    effective_config: Dict[str, Any]
    derived_from_id: Optional[PyUUID]
    created_at: datetime
