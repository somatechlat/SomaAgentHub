...
class SomaTraceSpanSummary(Base):
    ...
    attributes = Column(JSONB, nullable=False, default=dict)  # task_id, node_id, role_id, capsule_id, etc.
...
class EvaluationScenarioDefinition(Base):
    ...
    metrics_to_compute = Column(JSONB, nullable=False, default=list)  # e.g., pass@k, accuracy, cost
...
class EvaluationRun(Base):
    ...
    scenario_id = Column(UUID(as_uuid=True), ForeignKey("evaluation_scenario_definitions.id"), nullable=False, index=True)
    ...
    # Version tracking
    evaluated_version_set = Column(JSONB, nullable=False)  # Which blueprint, Capsule, agents tested
...
class EvaluationMetricRecord(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    evaluation_run_id = Column(UUID(as_uuid=True), ForeignKey("evaluation_runs.id"), nullable=False, index=True)
