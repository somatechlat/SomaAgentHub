...
@router.post(
    "/", response_model=BlueprintDefinitionResponse, status_code=status.HTTP_201_CREATED
)
...
async def list_blueprints(
    status: BlueprintStatus | None = Query(None),
    x_tenant_id: UUID = Header(..., alias="X-Tenant-ID"),
    service: BlueprintService = Depends(get_blueprint_service),
):
...
@router.post(
    "/plans", response_model=PlanSpecResponse, status_code=status.HTTP_201_CREATED
)
