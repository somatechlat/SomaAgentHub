...
from sqlmodel import Field, SQLModel, ARRAY, String, JSON, Column
...
@router.post("/capsules", response_model=CapsuleSpec, status_code=status.HTTP_201_CREATED)
async def create_capsule(capsule: CapsuleSpec, session: AsyncSession = Depends(get_session)):
    ...
    if not name or not version:
         raise HTTPException(status_code=400, detail="Capsule metadata must contain name and version")

    stmt = select(CapsuleModel).where(CapsuleModel.name == name, CapsuleModel.version == version)
    result = await session.execute(stmt)
    if result.scalar_one_or_none():
        raise HTTPException(status_code=409, detail=f"Capsule {name}:{version} already exists")
...
async def get_capsule(name: str, version: str, session: AsyncSession = Depends(get_session)):
