...
class AnalyzeProjectRequest(BaseModel):
    ...
    class IntakeRequestModel(BaseModel):
        ...
        class IntakeResponseModel(BaseModel):
            ...
            @router.post(
                "/{plan_id}/intake", response_model=IntakeResponseModel
            )
    ...
    answered = (
        payload.answer.get("answered", []) if payload.answer else []
    )
    answers_map = {}
    if (
        payload.question_id
        and payload.answer
        and "value" in payload.answer
    ):
        ...
        if payload.question_id not in answered:
            ...
            if (
                ...
                if validation_errors:
                    ...
                    next_step = await manager.next_step(
                        request_model
                    )
