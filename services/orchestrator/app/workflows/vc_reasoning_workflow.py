Temporal workflow implementing a Solver → Verifier → Corrector loop.
 ...
 workflow is triggered when the ``MultiAgentPattern.VC_REASONING`` pattern
 detected.  It creates a persistent ``VCEpisode`` record, then iterates up to
 ax_iterations`` (default 5) performing the following steps:
 ...
 --------------------------------------------------------------------------
 mports from the orchestrator package
 --------------------------------------------------------------------------
 ultiAgentPattern import removed as it is unused in this workflow.
 ...
 --------------------------------------------------------------------------
 ctivity implementations – these run in the Temporal worker process.
 --------------------------------------------------------------------------
 ...
 he emit_vc_step_activity function and its related imports have been removed per user request to eliminate parallel execution logic.
 ...
 --------------------------------------------------------------------------
 vent emission activity – emits a VC_STEP event after a step is persisted.
 --------------------------------------------------------------------------
 ...
 async def run(self, request: Dict[str, Any]) -> Dict[str, Any]:
     ...
     ------------------------------------------------------------------
     reate the episode record and record metrics.
     ------------------------------------------------------------------
 ...
 ncrement episode counter
 vc_episode_total.labels(tenant=tenant).inc()

 ------------------------------------------------------------------
 ain loop – solver → (optional) verifier → corrector.
 ------------------------------------------------------------------
 ...
 ------------------------------------------------------------------
 ark episode as completed and record duration metric.
 ------------------------------------------------------------------
 duration_seconds = time.time() - start_time
 vc_episode_duration_seconds.labels(tenant=tenant).observe(duration_seconds)
 pisode status update is optional for this minimal implementation.
 he caller can query the episode table later to see the final state.
