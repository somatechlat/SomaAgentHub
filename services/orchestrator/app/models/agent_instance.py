...
class AgentInstance(SQLModel, table=True):
    ...
    capsule_id: uuid.UUID | None = Field(
        default=None,
        foreign_key="capsules.capsule_id",
        description="Associated capsule ID",
    )

    tenant_id: uuid.UUID = Field(index=True, description="Tenant identifier")
    ...
    k8s_job_name: str | None = Field(
        default=None,
        max_length=100,
        description="Kubernetes job name (for batch agents)",
    )

    k8s_deployment_name: str | None = Field(
        default=None,
        max_length=100,
