        ...
        is module provides a richer implementation for the ``execute_capsule``
        tivity used by the ``capsule-run-workflow``.  The original stub executed a
        ngle Docker container based on ``payload.params['image']`` and returned a
        ring.  The canonical roadmap (see `docs/ROADMAP.md`) requires a *worker* that can:
