...
class RoleService:
    ...
    async def create_role_definition(
        self, role_create: RoleDefinitionCreate
    ) -> RoleDefinition:
    ...
    async def get_role_definition(
        ...
        result = await self.db.execute(
            select(RoleDefinition).where(
                RoleDefinition.id == role_id, RoleDefinition.tenant_id == tenant_id
            )
        )
    ...
    async def create_agent_binding(
        self, binding_create: AgentBindingCreate
    ) -> AgentBinding:
        """Create a binding between a role and an agent implementation"""
        # Validate role exists
        role = await self.get_role_definition(
            binding_create.role_id, binding_create.tenant_id
        )
        if not role:
            raise HTTPException(
    ...
    async def get_agent_binding(
        self, binding_id: UUID, tenant_id: UUID
    ) -> AgentBinding | None:
        """Get an agent binding by ID"""
        result = await self.db.execute(
            select(AgentBinding).where(
                AgentBinding.id == binding_id, AgentBinding.tenant_id == tenant_id
            )
        )
        return result.scalar_one_or_none()

    async def list_bindings_for_role(
        self, role_id: UUID, tenant_id: UUID
    ) -> list[AgentBinding]:
        """List all bindings for a specific role"""
        result = await self.db.execute(
            select(AgentBinding).where(
                AgentBinding.role_id == role_id, AgentBinding.tenant_id == tenant_id
            )
        )
        return result.scalars().all()
    ...
    async def create_session_binding(
        self, session_create: AgentSessionBindingCreate
    ) -> AgentSessionBinding:
        """Create a session binding for a workflow execution"""
        # Validate binding exists
        binding = await self.get_agent_binding(
            session_create.agent_binding_id, session_create.tenant_id
        )
        if not binding:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def close_session_binding(
    ...
    async def get_active_session_for_workflow(
        self, workflow_instance_id: UUID, role_id: UUID, tenant_id: UUID
