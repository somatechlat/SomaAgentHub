...
class ToolService:
    ...
    async def create_tool_definition(
        self, tool_create: ToolDefinitionCreate
    ) -> ToolDefinition:
    ...
    async def get_tool_definition(
        self, tool_id: UUID, tenant_id: UUID
    ) -> ToolDefinition | None:
        """Get a tool definition by ID"""
        result = await self.db.execute(
            select(ToolDefinition).where(
                ToolDefinition.id == tool_id, ToolDefinition.tenant_id == tenant_id
            )
        )
        return result.scalar_one_or_none()

    async def list_tool_definitions(
        self, tenant_id: UUID, tool_type: ToolType | None = None
    ) -> list[ToolDefinition]:
    ...
    async def create_mcp_server(
        self, server_create: MCPServerDefinitionCreate
    ) -> MCPServerDefinition:
    ...
    async def get_mcp_server(
        self, server_id: UUID, tenant_id: UUID
    ...
    async def record_invocation_start(
        self, record_create: ToolInvocationRecordCreate
    ) -> ToolInvocationRecord:
        """Record the start of a tool invocation"""
        # Validate tool exists
        tool = await self.get_tool_definition(
            record_create.tool_definition_id, record_create.tenant_id
        )
        if not tool:
            raise HTTPException(
    ...
    async def update_invocation_status(
        ...
        response_payload_inline: dict | None = None,
