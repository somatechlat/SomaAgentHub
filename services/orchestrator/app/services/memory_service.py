...
class MemoryService:
    ...
    async def create_binding_spec(
        self, spec_create: MemoryBindingSpecCreate
    ) -> MemoryBindingSpec:
    ...
    async def get_binding_spec(
        self, binding_id: UUID, tenant_id: UUID
    ) -> MemoryBindingSpec | None:
    ...
    async def get_binding_for_workflow(
        self, workflow_instance_id: UUID, tenant_id: UUID
    ) -> MemoryBindingSpec | None:
    ...
    async def record_operation(
        self, op_create: MemoryOperationRecordCreate
    ) -> MemoryOperationRecord:
        """Record a memory operation (read/write)"""
        # Validate binding exists for context
        binding = await self.get_binding_for_workflow(
            op_create.workflow_instance_id, op_create.tenant_id
        )
        if not binding:
    ...
    async def list_operations_for_workflow(
        self, workflow_instance_id: UUID, tenant_id: UUID
