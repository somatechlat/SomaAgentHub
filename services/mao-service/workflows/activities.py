...
async def create_workspace(config: dict[str, Any]) -> str:
    ...
    try:
        ...
        activity.logger.info(f"Workspace ready: {workspace_id} on port {host_port}")

        # Store workspace metadata
        await _store_workspace_metadata(
            workspace_id,
            {
...
async def provision_git_repo(workspace_id: str, repo_url: str) -> dict[str, Any]:
    ...
    try:
        ...
        if exec_result.exit_code != 0:
            raise RuntimeError(f"Git clone failed: {exec_result.output.decode()}")

        activity.logger.info("Repository cloned successfully")
...
async def execute_capsule(task_config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        # Get workspace metadata (needed for potential future extension)
        _ = await _get_workspace_metadata(workspace_id)
        ...
        activity.logger.info(f"Capsule execution completed in {execution_time:.2f}s")
...
@activity.defn
async def notify_completion(webhook_urls: list[str], payload: dict[str, Any]) -> None:
    """
