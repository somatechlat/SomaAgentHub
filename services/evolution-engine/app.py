...
# In‑memory store for suggestions generated by ``evolve_capsule``.  The
# ``evolve_capsule`` endpoint populates this dict so that ``get_pending_suggestions``
# can retrieve them without a database.
_suggestions_store: dict[str, list[ImprovementSuggestion]] = defaultdict(list)
...
def get_pending_suggestions(capsule_id: str):
    """Return pending improvement suggestions for a capsule.

    In a production environment this would query a persistent store.  For the
    purposes of the current code‑base we keep an **in‑memory store** that is
    populated when ``evolve_capsule`` is called.  This satisfies the VIBE rule
    of providing a real implementation (no ``raise NotImplementedError``) while
    remaining lightweight and testable.
    """

    # ``_suggestions_store`` is defined lazily the first time the module is
    # imported.  If no suggestions have been generated yet, an empty list is
    # returned – matching the expected FastAPI response model.
    return _suggestions_store.get(capsule_id, [])
@app.get("/health")
def health_check():
