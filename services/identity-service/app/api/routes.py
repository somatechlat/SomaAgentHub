...
async def get_key_manager(request: Request) -> KeyManager:
    ...
    if key_manager is None:
        raise HTTPException(
            ...
            detail="Key manager unavailable",
        )
    return key_manager
    ...
    if payload.capabilities:
        ...
        if missing:
            raise HTTPException(
                ...
                detail=f"User lacks capabilities: {missing}",
            )
    ...
    if tenant_id:
        ...
        if current_hash and constitution_hash != current_hash:
            raise HTTPException(
                ...
                detail="Constitution hash mismatch",
            )
...
async def get_training_lock(
    ...
    if lock is None:
        return TrainingLockStatus(
            tenant_id=tenant_id, locked=False, requested_by="system"
        )  # Added requested_by to satisfy model
    return lock
