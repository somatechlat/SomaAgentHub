...
def deploy_to_kubernetes(manifest: str, kubeconfig_path: str | None = None):
...
def seed_knowledge_base(instance_id: str, knowledge_data: dict[str, Any]):
...
def configure_identity(instance_id: str, organization_id: str):
...
def initialize_constitution(instance_id: str, organization_id: str):
...
async def provision_instance(request: ProvisionRequest):
    ...
    try:
        ...
        _tf_config = generate_terraform_config(request)
        ...
        _k8s_manifest = generate_kubernetes_manifest(
            request,
...
@app.get("/instances")
def list_instances(organization_id: str | None = None):
