...
readme: str | None = Field(None, description="Markdown README content")
...
# Get rating stats
rating_stats = (
db.query(func.avg(CapsuleRating.rating), func.count(CapsuleRating.id))
.filter(CapsuleRating.capsule_id == capsule.id)
.first()
)
...
# Get latest version
latest_version = (
db.query(CapsuleVersion)
.filter(CapsuleVersion.capsule_id == capsule.id)
...
capsule.updated_at = datetime.now(UTC)
db.commit()
db.refresh(capsule)
