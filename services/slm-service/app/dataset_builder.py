...
class TrainingExample:
    ...
    quality: DataQuality
    rating: float | None  # User rating if available
    task_category: str
    created_at: datetime
...
class DatasetBuilder:
    ...
    def filter_by_task_category(self, examples: list[TrainingExample], categories: list[str]) -> list[TrainingExample]:
        ...
        Returns:
            Filtered examples
        """
        filtered = [ex for ex in examples if ex.task_category in categories]

        logger.info(f"Filtered to {len(filtered)} examples in {categories}")
        return filtered

    def balance_dataset(
        self, examples: list[TrainingExample], samples_per_category: int = 100
    ) -> list[TrainingExample]:
        """
