...
from services.common.config.unified_config import config_manager, DeploymentMode
...
class DockerComposeGenerator:
    ...
    def _generate_database_services(self) -> Dict[str, Any]:
        ...
        services["database-cluster"] = {
            ...
            "environment": {
                "POSTGRES_USER": "somaagent", "POSTGRES_PASSWORD": "somaagent",
                "POSTGRES_DB": "somaagent", "POSTGRES_INITDB_ARGS": "--auth-host=scram-sha-256"
            },
            "ports": ["5432:5432"],
            "volumes": ["postgres_data:/var/lib/postgresql/data"],
            "networks": ["somagenthub-network"],
            "deploy": {"resources": {"limits": {"memory": db_config.memory_limit, "cpus": db_config.cpu_limit}}},
            "healthcheck": {
    ...
    def _generate_application_services(self) -> Dict[str, Any]:
        ...
        for service_name in self.config_manager.get_enabled_services():
            ...
            services[service_name] = {
                "build": {"context": f"./services/{service_name}", "dockerfile": "Dockerfile"},
    ...
    def _get_service_port(self, service_name: str) -> int:
        port_map = {
            "gateway-api": 10000, "workflow-engine": 10001, "ai-services": 10022,
            "capsule-manager": 10002, "data-services": 10003, "governance-services": 10004,
