...
class SomaAgentTestReporter:
    ...
    def run_ci_tests(self):
        ...
        "images",
        "--filter",
        "reference=ghcr.io/somatechlat/soma-*",
        ],
        ...
        for test in tests:
            try:
                result = subprocess.run(
        test["command"], capture_output=True, text=True, timeout=30
        )
        ...
        test_results.append(test_result)
            ...
            test_results.append(
        {"name": test["name"], "success": False, "error": str(e)}
        )
        ...
        "success_rate": sum(1 for t in test_results if t.get("success", False))
        / len(tests)
        * 100,
        }
    ...
    def generate_recommendations(self):
        ...
        # Kubernetes recommendations
                    if k8s.get("deployment_success_rate", 0) == 100:
                        ...
                        "✅ Kubernetes deployment is fully successful - all pods running!"
                        )
                        elif k8s.get("deployment_success_rate", 0) > 50:
                            recommendations.append(
                            "⚠️ Partial Kubernetes deployment - investigate failing pods"
                            )
                            else:
        ...
        # CI recommendations
                                if ci.get("success_rate", 0) == 100:
                                    recommendations.append(
                                    "✅ All CI/CD tests passing - deployment pipeline healthy"
                                    )
                                    else:
        ...
        # Service health recommendations
                                        ...
                                        s
                                        for s, data in health.items()
                                        if "missing_dependencies" in data.get("issues", [])
                                        ]
                                        if services_with_issues:
                                            recommendations.append(
                                            f"🔧 Rebuild containers for services with missing dependencies: {', '.join(services_with_issues)}"
                                            )
                                            ...
                                            if services_running:
                                                recommendations.append(
                                                f"🚀 {len(services_running)} services successfully started and running"
                                                )

        # Overall platform status
                                                if (
                                                k8s.get("deployment_success_rate", 0) >= 80
                                                and ci.get("success_rate", 0) >= 80
                                                and len(services_running) > 5
                                                ):
                                                    recommendations.append(
                                                    "🎉 SomaAgent platform is operational and ready for development!"
                                                    )
    ...
    def print_final_report(self):
                                                            ...
                                                            print(
                                                            f"   • Pods Running: {k8s.get('running_pods', 0)}/{k8s.get('total_pods', 0)}"
                                                            )
        ...
        # Overall assessment
                                                                                if (
                                                                                k8s.get("deployment_success_rate", 0) >= 100
                                                                                and ci.get("success_rate", 0) >= 80
                                                                                ):
