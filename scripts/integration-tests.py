...
class SomaAgentTester:
    ...
    async def test_service_endpoint(
        self, service: ServiceTest, endpoint: str
    ) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                return {
                    ...
                    "response_preview": (
                        str(response.text)[:200] if response.text else ""
                    ),
                }
        except Exception as e:
            return {
    ...
    async def run_integration_tests(self):
        ...
        for result in results:
            ...
            print(
                f"{status} {result['name']} - {len(result['endpoints'])} endpoints tested"
            )
            print(f"      Response time: {result['total_response_time']:.3f}s")

            # Show failed endpoints
            failed_endpoints = [
                ep for ep in result["endpoints"] if not ep.get("success", False)
            ]
            if failed_endpoints:
    ...
    def run_ci_tests(self):
        ...
        tests = [
            (
                ...
                "helm template soma-agent-hub ./k8s/helm/soma-agent --dry-run",
            ),
            (
