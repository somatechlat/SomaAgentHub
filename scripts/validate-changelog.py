...
if clean_tag not in changelog_versions:
    self.warnings.append(
        f"Git tag '{clean_tag}' not found in changelog"
    )
...
self.warnings.append(
    f"Consider adding '### {subsection}' subsection to Unreleased"
)
...
r"### (Added|Changed|Deprecated|Removed|Fixed|Security)(.*?)(?=### |\Z)",
section_content,
re.DOTALL,
)
...
if not subsection_content.strip():
    self.warnings.append(
        f"Empty {subsection_name} section in version {version}"
    )
